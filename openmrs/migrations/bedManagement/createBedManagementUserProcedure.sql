...
CREATE PROCEDURE create_bed_management_user( user_gender varchar(50),
  ...
  BEGIN
    ...
    INSERT INTO users (system_id, username, password, creator, date_created, changed_by, person_id,retired,uuid) VALUES (user_username, user_username,(SELECT SHA2('test',512)),(SELECT SHA2('salt',512)),6,now(),6,(SELECT person_id FROM person where uuid=@puuid),0,uuid());
    INSERT INTO user_role VALUES ((SELECT max(user_id) FROM users),"Registration-App"),((SELECT max(user_id) FROM users),"InPatient-App");
